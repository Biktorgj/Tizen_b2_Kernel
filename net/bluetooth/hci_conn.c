...
static void hci_conn_idle(struct work_struct *work)
{
	...
	if (!test_and_set_bit(HCI_CONN_MODE_CHANGE_PEND, &conn->flags)) {
		...
		BT_INFO("Send Sniff mode packet");
...
{
	struct hci_cp_le_encrypt cp;

	memset(&cp, 0, sizeof(cp));
	memcpy(cp.key, key, 16);
	swap128(plaintext_data, cp.plaintext_data);

	return hci_send_cmd(hdev, HCI_OP_LE_ENCRYPT, sizeof(cp), &cp);
}
#endif
