...
static int fallbacks[MIGRATE_TYPES][4] = {
	...
	[MIGRATE_CMA]         = { MIGRATE_CMA,         MIGRATE_RESERVE },
#else
						...
						int migratetype)
{
	...
	if (migratetype == MIGRATE_MOVABLE && nr_cma_free &&
	    (nr_free < 2 * nr_cma_free || nr_free < 2 * low_wmark_pages(zone)))
...
migrate:
	 ...
	 */
	__reclaim_pages(zone, GFP_HIGHUSER_MOVABLE, 1);
...
void free_contig_range(unsigned long pfn, unsigned nr_pages)
{
	for (; nr_pages--; ++pfn) {
		struct page *page = pfn_to_page(pfn);
		WARN(page_count(page) != 1, "page %p still in use!\n", page);
		__free_page(page);
	}
}
#endif
