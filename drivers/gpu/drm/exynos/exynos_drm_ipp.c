...
static int ipp_subdrv_probe(struct drm_device *drm_dev, struct device *dev)
{
	...
	list_for_each_entry(ippdrv, &exynos_drm_ippdrv_list, drv_list) {
		...
		mutex_init(&ippdrv->cwq_lock);
		ippdrv->suspended = true;
		ctx->num_drv++;
...
{
	int ret = 0, pm_ref = atomic_read(&ippdrv->dev->power.usage_count);
...
err:
	DRM_ERROR("pm[%d]ref[%d %d]\n",
		!ipp_runtime_suspended(ippdrv->dev), pm_ref,
		atomic_read(&ippdrv->dev->power.usage_count));
